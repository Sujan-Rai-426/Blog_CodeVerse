...
useEffect(() => {
  ...
  function smoothAutoScroll() {
    if (autoScrollEnabled && !isDragging) {
      carousel.scrollTo({
        left: carousel.scrollLeft + autoScrollSpeed * scrollDirection,
        behavior: "smooth",
      });
      if (atEnd()) scrollDirection = -1;
      else if (atStart()) scrollDirection = 1;
    }
    timeoutId = setTimeout(smoothAutoScroll, 25); // ~40fps
  }

  // Start after layout settles (important for iOS)
  const startTimeout = setTimeout(smoothAutoScroll, 1000);
